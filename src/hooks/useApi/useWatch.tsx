...
import {UpcomingMovieType} from '@types';

export function useWatch() {
  const [movies, setMovies] = useState<UpcomingMovieType[]>([]);
  ...
  const fetchMovies = useCallback(async () => {
    try {
      ...
      if (fetchNext === 1) setInitialLoading(true);
      else setLoading(true);

      const results = await apiClient.fetchUpcomingMovies(fetchNext);
      ...
      setMovies(prev => {
        ...
        const uniqueMovies = newMovies.filter(
          movie => !existingMovieIds.includes(movie.id),
        );
